...
1. It is done in lines 11-18 of dataSet1.R8
...
1.	To get the number of rows of the new matrix, use nrow(transposedData). To get the number of columns of the new matrix, use ncol(transposedData).

2. ```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
...
1. In dataSet1.R line 9, change the code into startDate = "2001-01-01". Then re-run the lines 11-18 to download new data and work with the data since 2001-01-01.
...
1. The sum is 20. 'myData != StockReturns[1:20,]' compares if the value in 'myData' differs from 'StockReturn[1:20,]'. If they are different, the logic value is 1. If they are the same, the logic value is 0. The command 'sum(myData != StockReturns[1;20,]) adds up the comparaison values of all 20 days and shows they are different in 20 days.

2.	There would be an error message if I run the command 'myData + StockReturns[1:40,]'. Because class(myData) is data.frame and class(StockReturns) is matrix. They are not the same format and cannot be added up directly.
