...
1. We need 6 principal components to explain at least 50% of the variance in this data.

2. 5 yr T-Notes US (0.93), 10 yr T-Notes (0.93), Treasury Bonds (0.86), 2 yr T-Note US (0.85), Canadian Bond (0.79), Eurodollar (0.74).

3. We can interpret the first 10 components. Their Eigenvalues are more than 1 and have a cumulative percentage of expalined variance 62.61%. They are principal components.

4.a. **Before crisis**
```r pre_crisis_data  = futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-10-01",]
```
...
Pre_Crisis_m1<-gvisTable(Pre_Crisis_show_data,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(Pre_Crisis_show_data)+1)),allowHTML=TRUE,page='disable'))
print(Pre_Crisis_m1,'chart')
```
<br>

4.b. **During crisis**
```r crisis_data  = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01",]
```
  
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
Crisis_Variance_Explained_Table_results<-PCA(crisis_data, graph=FALSE)
Crisis_Variance_Explained_Table<-cbind(paste("component",1:ncol(crisis_data),sep=" "),Crisis_Variance_Explained_Table_results$eig)
Crisis_Variance_Explained_Table<-as.data.frame(Crisis_Variance_Explained_Table)
colnames(Crisis_Variance_Explained_Table)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```
  
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
Crisis_show_data = data.frame(Crisis_Variance_Explained_Table)
Crisis_m1<-gvisTable(Crisis_show_data,options=list(width=1920, height=min(400,27*(nrow(Crisis_show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(Crisis_m1,'chart')
```
<br> 
...
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
Crisis_eigenvalues  <- Crisis_Variance_Explained_Table[,2]
```
...
Crisis_Line         <- gvisLineChart(as.data.frame(Crisis_df), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Crisis_Line, 'chart')
```
...
colnames(Crisis_Rotated_Factors)<-paste("Component",1:ncol(Crisis_Rotated_Factors),sep=" ")

Crisis_sorted_rows <- sort(Crisis_Rotated_Factors[,1], decreasing = TRUE, index.return = TRUE)$ix
Crisis_Rotated_Factors <- Crisis_Rotated_Factors[Crisis_sorted_rows,]
Crisis_Rotated_Factors[abs(Crisis_Rotated_Factors) < 0.3]<-NA
```

```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
Crisis_show_data <- Crisis_Rotated_Factors 
Crisis_show_data<-cbind(rownames(Crisis_show_data),Crisis_show_data)
colnames(Crisis_show_data)<-c("Variables",colnames(Crisis_Rotated_Factors))
Crisis_m1<-gvisTable(Crisis_show_data,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(Crisis_show_data)+1)),allowHTML=TRUE,page='disable'))
print(Crisis_m1,'chart')
``` 
<br>

5. During bull year, there are 15 components with Eigenvalue > 1 and explain 69.95% variance. The component 1 alone explains 17.49%.
During bear year, there are 13 components with Eigenvalue > 1 and explain 75.44% variance. The component 1 alone explains 29.78%.
This implies the more components with Eigenvalue > 1, the better the financial market is and each component explains less variance which means they are stable.
During bear, commodity such as Soybean, Soybean oil, corn and wheat became more important (No.2 component) than others.
...
1. The code defines a zero variable 'signal_used' firtly then use moving average and ad their sign(+ or -) into this variable to vote long or short. Then it uses the function to scale the data and make sure we invest $1 every day. After, it creates the returns of the strategy for each futures time series and predicts future strategy by using the signal from 2 days ago and transforms into trading strategy.

2. The following code defines a shorter term (250 -> 200) and a smaller interval (20 -> 10).The result is similar.


```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,200,by=10)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
