...
1. The correaltion between Q1_13 and Q1_14 is 0.64. The correlation between Q1_16 and Q1_17 is 0.63. The correlation betwwen Q1_27 and Q1_28 is 0.62. They make sense because they share a similar attribute/element between two correlated factors.

2. Correaltion between 13 and 14 means when someone buys a latest and greatest model, he or she tends to buy lastest and greatest accessories to match. Correlation between 16 and 17 means because I am knowledgeable about boat so people tend to approach me for advice, which is reasonable. Correlation between 27 and 28 means becasue boating is my true passion in my life, so it is the number 1 thing I want to do with my spare time.
...
1. This table use the PCA method to calculate the Eigenvalue of each factor and respective weight. The plot visulize the table. It indciate that there 5 principal components and they explain 52.72% of total variance.

2. The red line means Eigenvalue equals 1. It shows there are 5 factors with Eigenvalue more than 1 and there 5 are principal components.The first one explains the biggest weight of variance. The plotted line could not be increasing, because the factors are in decreasing order of variance explanation.

3. We would prefer to see les components which explain more variance. It means we can use less components to explain more varaince so that we can simplfy the model and calculation while garantee the precision.
...
1. The first couple of factors explain more the reason why people buy a boat. For example, the first factor related to the wealth of a person. This is true and have business senses because only rich people can buy a boat.

2. As there are 5 principal components, we choose 5. Statistically, these 5 are principal components with Eigenvalue more than 1 and explain more than 50% of information. These 5 components regroup the information each factor carries and put together. On a business base, there 5 components are the 5 most important attributes or concerns when people buy a boat.

3. The factors I selected explain most reasons people buy a boat. The first factor relates to wealth and social status. The second factor relates to lifestyle. The third one relates to reputation. The 4th one relates to quality of a boat or how to choose a good boat. The 5th one concerns more about price.

4. From a big amount of data, we can regroup and find the most important factors we carry most information of all data. By using data analysis, we can draw business interpretations from statistical interpretations. 
...
1. There are 3 segments because dendogram has 3 segments.

2. Segment 1: Unenthusiastic social boaters. Segment 2: Future boat buyer. Segment 3: Boat enthusiasts with strong associations.

3. If I change the number of factors and re-do the whole analysis, the number of segments will change too. If a smaller number, we will have less segments so the segmentation will not be specified. If a bigger number, we will have more segments which means the segmentation will be detailed.
